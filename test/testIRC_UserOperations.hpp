...
TEST(IRC_UserOperations, listProvideChannelList) {
	IRC_Logic logic("password");
	registerMembersAndJoinToChannel(&logic, 0, 1, "#chancool");
	registerMembersAndJoinToChannel(&logic, 1, 2, "#chancooler");
	emptyQueue(&logic.getMessageQueue());

	logic.processRequest(0, "LIST \r\n");
