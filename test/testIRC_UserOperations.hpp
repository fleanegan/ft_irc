...
TEST(IRC_UserOperations, receivingPingReturnsPong){
	...
	ASSERT_TRUE(responseContains(logic.getMessageQueue().back().content, "nonEmptyToken"));
}
...
TEST(IRC_UserOperations, receivingWhoIsShouldSendInformationsAboutUser){
	...
	ASSERT_TRUE(responseContains(logic.getMessageQueue().back().content, RPL_WHOISUSER));
}
...
TEST(IRC_UserOperations, WhoWasReturnsInfoOnPreviouslyRegisteredUser) {
	...
	ASSERT_TRUE(responseContains(logic.getMessageQueue().back().content, "nick1"));
	ASSERT_TRUE(responseContains(logic.getMessageQueue().back().content, RPL_WHOWASUSER));
}
...
TEST(IRC_UserOperations, whoWasOnUnregisteredNicknameReturnsError){
	...
	ASSERT_TRUE(responseContains(logic.getMessageQueue().back().content, ERR_WASNOSUCHNICK));
}
