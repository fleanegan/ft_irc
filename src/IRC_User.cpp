...
IRC_User::IRC_User(int fd) : hostName("127.0.0.1"), isAuthenticated(false), fd(fd){
}

IRC_User::IRC_User(const std::string &nick, const std::string &name, const int &fd):
		nick(nick), userName(name), hostName("127.0.0.1"), isAuthenticated(false), fd(fd) {
}
...
IRC_User::UserIterator IRC_User::findUserByFdInVector(std::vector<IRC_User> *users, int fd) {
...
IRC_User::UserIterator IRC_User::findUserByNickInVector(std::vector<IRC_User> *users, const std::string &nick) {
	for (UserIterator it = users->begin();
		 it != users->end(); it++) {
		if (it->nick == nick)
			return it;
