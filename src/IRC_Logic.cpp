...
void IRC_Logic::processNoticeMessage(
		IRC_User *user, const std::vector<std::string> &splitMessageVector) {
...
void IRC_Logic::appendRecipients(
		...
		bool replyToErrors) {
	bool entityFound;
	std::vector<std::string> targets = splitOnToken(splitMessageVector[1], ",");

	for (std::vector<std::string>::iterator target = targets.begin();
			target != targets.end(); ++target) {
		entityFound = appendMatchingSingleRecipients(user, recipients,
				*target) ||
...
bool IRC_Logic::appendMatchingSingleRecipients(
		const IRC_User &user, std::queue<int> *recipients,
	...
	for (IRC_User::iterator it = _users.begin(); it != _users.end(); ++it) {
		if ((user.nick != it->nick &&
			...
			recipients->push(it->fd);
					...
					isMatchingWildcardExpression(it->userName,
						target.substr(0, target.find('@')))) &&
				isMatchingWildcardExpression(it->hostIp,
...
bool IRC_Logic::isValidRecipientMask(
		const IRC_User &user, const std::string &target) {
	u_int64_t lastMultiWildcardPosition = target.rfind('*');
	u_int64_t lastPointPosition = target.rfind('.');
	u_int64_t lastSingleWildcardPosition = target.rfind('?');
	bool multiWildAfterLastPoint =
		lastMultiWildcardPosition != std::string::npos &&
		lastMultiWildcardPosition > lastPointPosition;
	bool singleWildAfterLastPoint =
		lastSingleWildcardPosition != std::string::npos &&
		lastSingleWildcardPosition > lastPointPosition;
	bool maskHasNoPoint =
		target.substr(target.find('@') + 1).find('.') == std::string::npos;
...
void IRC_Logic::updateNickInAllChannels(
	...
	for (IRC_Channel::iterator it = _channels.begin();
			it != _channels.end(); ++it) {
		if (it->isUserInChannel(*user)) {
			it->broadCastToOtherMembers(concatenateContentFromIndex(
						0, splitMessageVector), *user, &_messageQueue);
...
void IRC_Logic::processOperMessage(IRC_User *user,
	...
	} else if (splitMessageVector[1] != _operName ||
			splitMessageVector[2] != _operPassword) {
		reply.content = generateResponse(ERR_PASSWDMISMATCH,
				"Password incorrect");
	} else {
...
void IRC_Logic::processKillMessage(
		IRC_User *user, const std::vector<std::string> &splitMessageVector) {
	IRC_Message reply(user->fd, "", "");
