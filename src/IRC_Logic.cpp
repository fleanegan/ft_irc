...
void IRC_Logic::appendRecipients(
	...
	std::queue<std::string> expandedRecipientNicks;
	std::vector<IRC_User> recipientCandidates;
	if (user->nick != splitMessageVector[1])
		expandedRecipientNicks.push(splitMessageVector[1]);
	if (expandedRecipientNicks.empty() == false && getUserByNick(expandedRecipientNicks.front()) != _users.end())
		fetchSingleRecipient(user->fd, splitMessageVector, recipients, replyToErrors);
	else if (expandedRecipientNicks.empty() == false && (IRC_Channel::isChannelPrefix(expandedRecipientNicks.front()) \
 || getChannelByName(expandedRecipientNicks.front()) != _channels.end()))
		fetchChannelRecipients(*user, expandedRecipientNicks.front(), recipients, replyToErrors);
	else if (replyToErrors)
...
void
IRC_Logic::fetchSingleRecipient(int fd, const std::vector<std::string> &splitMessageVector, std::queue<int> *recipients,
								bool replyToErrors) {
...
void
IRC_Logic::fetchChannelRecipients(const IRC_User &user, const std::string &channelName, std::queue<int> *recipients,
								  bool replyToErrors) {
	IRC_Message reply(user.fd, "", "");
	...
	if (channelCandidate == _channels.end()) {
		...
		channelCandidate->appendRecipientFdsForSender(user, recipients);
	}
	if (!reply.content.empty() && replyToErrors)
		appendMessage(reply);
}
