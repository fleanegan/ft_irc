...
void IRC_Logic::processPrivMsgMessage(IRC_User *user, const std::vector<std::string> &splitMessageVector) {
    sendContent(user, splitMessageVector, true);
}

void IRC_Logic::processNoticeMessage(IRC_User *user, const std::vector<std::string> &splitMessageVector) {
    sendContent(user, splitMessageVector, false);
}

void IRC_Logic::sendContent(
        const IRC_User *user,
        const std::vector<std::string> &splitMessageVector,
        bool replyToErrors) {
...
void IRC_Logic::appendRecipients(const IRC_User *user, const std::vector<std::string> &splitMessageVector,
                                 std::queue<int> &recipients,
                                 bool replyToErrors) {
    if (splitMessageVector[1][0] == '#')
        fetchChannelRecipients(*user, splitMessageVector[1], &recipients, replyToErrors);
    else
